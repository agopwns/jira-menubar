...
## 터미널에서 보기 (선택): tickets CLI

메뉴바 위젯과 같은 설정 파일을 그대로 쓰는 터미널 대시보드도 있습니다. 별도 설정 없이 심링크만 걸면 됩니다.

```
ln -sf "<레포 경로>/bin/tickets" ~/.local/bin/tickets
tickets            # 한 번 출력
tickets -w 120     # 2분마다 갱신되는 실시간 창
```

남이 움직인/즉시/진행 중/테스트/계획 중 순으로 섹션이 나뉘고, 로컬 git worktree에 체크아웃된 티켓에는 `●`, 그 워크트리 안에서 tmux 작업 중이면 `▶` 마커가 붙습니다. 자세한 옵션은 `tickets --help`와 README의 "Terminal ticket dashboard" 섹션을 참고하세요.
